...
func (i *PreservationIncident) RoundDetail(number int) (TreatmentRoundEvidence, error) {
	...
	for _, round := range i.RoundHistory {
		if round.RoundNumber == number {
			copy := round
			// Keep the historical evidence slices attached to the aggregate.  The
			// records are treated as immutable snapshots by callers.
			copy.Treatments = round.Treatments
			copy.Verifications = round.Verifications
