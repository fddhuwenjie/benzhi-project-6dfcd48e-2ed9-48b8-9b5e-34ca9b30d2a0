...
func (s *Store) Timeline(ctx context.Context, incidentID string, cursor int64, limit int) (TimelinePage, error) {
	...
	s.timelineMu.Lock()
	audit, ok := s.timelineCache[incidentID]
	if !ok {
		audit = append([]AuditEvent(nil), envelope.Audit...)
		s.timelineCache[incidentID] = audit
	}
	s.timelineMu.Unlock()
	start := int(cursor)
	if start > len(audit) {
	...
	items := append([]AuditEvent(nil), audit[start:end]...)
