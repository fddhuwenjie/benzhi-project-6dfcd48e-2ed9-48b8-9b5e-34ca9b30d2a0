...
func (s *Service) BoundaryPreflight(ctx context.Context, incidentID string, cmd BoundaryPreflightCommand) (BoundaryPreflightResponse, error) {
	...
	cacheKey := fmt.Sprintf("%s:%d:%q", incidentID, incident.Revision, cmd.ExpectedMediaIDs)
	for _, entry := range s.preflightCache {
		if entry.key != cacheKey {
			continue
		}
		var cached BoundaryPreflightResponse
	...
	response := BoundaryPreflightResponse{IncidentID: incidentID, Revision: incident.Revision, BoundaryPreflightResult: result}
	if encoded, encodeErr := json.Marshal(response); encodeErr == nil {
		s.preflightCache = append(s.preflightCache, preflightCacheEntry{key: cacheKey, response: encoded})
	}
	return response, nil
}
